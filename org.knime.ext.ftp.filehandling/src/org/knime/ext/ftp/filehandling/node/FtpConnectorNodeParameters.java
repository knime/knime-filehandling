...
import org.knime.ext.ftp.filehandling.node.FtpConnectorNodeParameters.AuthenticationParameters.FtpFileSystemConnectionProvider;
...
final class FtpConnectorNodeParameters implements NodeParameters {
    ...
    private final static NodeLogger LOGGER = NodeLogger.getLogger(FtpConnectorNodeParameters.class);
    ...
    @WithCustomFileSystem(connectionProvider = FtpFileSystemConnectionProvider.class)
    ...
    static class WorkingDirectoryValidator extends SimpleValidation<String> {
        ...
        public void validate(final String currentValue) throws InvalidSettingsException {
            ...
            if (!currentValue.strip().startsWith(FtpFileSystem.PATH_SEPARATOR)) {
                throw new InvalidSettingsException(
    ...
    static final class AuthenticationParameters implements NodeParameters {
        ...
        enum AuthenticationMethod {
            @Label(value = "Username & password", description = """
                    ...
                    In the later case the password will be persistently stored (in encrypted form) with the workflow.""")
            USERNAME_PASSWORD,
        ...
        static class UserPasswordValidator implements CustomValidationProvider<LegacyCredentials> {
            ...
            public ValidationCallback<LegacyCredentials> computeValidationCallback(
                ...
                return value -> {
                    final var name = value.toCredentials(getCredentialsProvider(parametersInput)).getUsername();
                    CheckUtils.checkSetting(StringUtils.isNoneBlank(name), "Username must be specified.");
                };
            }
        }
    ...
    public void validateOnConfigure(final CredentialsProvider provider) throws InvalidSettingsException {
        ...
        if (!m_workingDirectory.strip().startsWith(FtpFileSystem.PATH_SEPARATOR)) {
            throw new InvalidSettingsException("Working directory must be absolute.");
        }
    ...
    /**
     * Convert parameters into a {@link FtpFSConnectionConfig} instance.
     *
