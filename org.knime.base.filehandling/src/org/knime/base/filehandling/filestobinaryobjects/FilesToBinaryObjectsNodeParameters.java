...
class FilesToBinaryObjectsNodeParameters implements NodeParameters {
    ...
    @Widget(title = "Append or replace",
        description = "Determines whether the binary object column is appended to the table or replaces the URI "
            + "column.")
    @ValueSwitchWidget
    ...
    @Widget(title = "New column name",
        description = "The name for the newly appended binary object column. Only relevant when \"Append\" is "
            + "selected.")
    ...
    private static final class AppendOrReplacePersistor implements NodeParametersPersistor<AppendOrReplace> {
        ...
        public AppendOrReplace load(final NodeSettingsRO settings) throws InvalidSettingsException {
            final String legacyValue = settings.getString(CONFIG_KEY);
            try {
                return AppendOrReplace.fromLegacyValue(legacyValue);
            } catch (final IllegalArgumentException e) {
                throw new InvalidSettingsException(createInvalidSettingsExceptionMessage(legacyValue), e);
            }
        }

        private static String createInvalidSettingsExceptionMessage(final String name) {
            var values = Arrays.stream(AppendOrReplace.values()) //
                    .map(AppendOrReplace::getLegacyValue) //
