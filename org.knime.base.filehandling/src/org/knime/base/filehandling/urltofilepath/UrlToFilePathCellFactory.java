...
class UrlToFilePathCellFactory extends AbstractCellFactory {
    ...
    private boolean m_isInputURIValue;

    /**
     * Creates new instance of <code>UrlToFilePathCellFactory</code>.
     *
     * @param colIndex The index of the column containing url strings.
     * @param isInputURIValue if true the values are {@link URIDataValue} compatible, otherwise just Strings.
    ...
    UrlToFilePathCellFactory(final int colIndex,
            final boolean isInputURIValue,
            final boolean failOnInvalidURL,
            final boolean failOnNonExistsingFile, final DataColumnSpec[] newColSpecs) {
        super(newColSpecs);

        m_isInputURIValue = isInputURIValue;
        m_colIndex = colIndex;
    ...
    public DataCell[] getCells(final DataRow row) {
        ...
        // if not missing try to convert url string
            ...
            try {
                if (m_isInputURIValue) {
                    URI uri = ((URIDataValue)cell).getURIContent().getURI();
                    url = uri.toURL();
                } else {
                    url = new URL(((StringValue)cell).getStringValue());
                }
            } catch (MalformedURLException | IllegalArgumentException e) {

                // if url string is not valid fail if specified
                if (m_failOnInvalidURL) {
                    throw new IllegalArgumentException("URL " + cell.toString() + " is not valid!");
