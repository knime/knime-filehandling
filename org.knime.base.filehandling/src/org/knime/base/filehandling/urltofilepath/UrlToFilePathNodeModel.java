 ...
 */
class UrlToFilePathNodeModel extends SimpleStreamableFunctionNodeModel {
     ...
     */
    @Override
    protected ColumnRearranger createColumnRearranger(final DataTableSpec dataSpec) throws InvalidSettingsException {
        //
        /// SPEC CHECKS
        //
        ...
        int selectedColIndex = dataSpec.findColumnIndex(m_stringColMode.getStringValue());
        CheckUtils.checkSetting(selectedColIndex >= 0, "Column \"%s\" is not available", selectedColName);
        DataColumnSpec selectedCol = dataSpec.getColumnSpec(selectedColIndex);
        DataType selectedColType = selectedCol.getType();
        CheckUtils.checkSetting(selectedColType.isCompatible(StringValue.class)
            || selectedColType.isCompatible(URIDataValue.class), "Column \"%s\" is neither a string nor URI column: %s",
            selectedColName, selectedColType.toPrettyString());
        ...
        UrlToFilePathCellFactory cellFac = new UrlToFilePathCellFactory(
                selectedColIndex, selectedColType.isCompatible(URIDataValue.class), failOnInvalidSyntax,
                failOnInvalidLocation, newColsSpecs);
