 ...
 */
class PNGsToBinaryObjectsNodeModel extends NodeModel {
     ...
     */
    protected PNGsToBinaryObjectsNodeModel() {
        super(1, 1);
    ...
    private DataCell createBOCell(final DataRow row, final DataTableSpec spec,
        ...
        if (!oldCell.isMissing()) {
            ...
            try {
                value.getImageContent().save(out);
                cell = bocellfactory.create(new ByteArrayInputStream(out.toByteArray()));
            } catch (IOException e) {
                // ignore
            }
        }
    ...
    private void checkSettings(final DataTableSpec inSpec) throws InvalidSettingsException {
        ...
        NodeUtils.checkColumnSelection(inSpec, "Binary object", selectedColumn, PNGImageValue.class);
