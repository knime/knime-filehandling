...
/**
 * Helper class that provides common code for the dialog and node model to
 * create {@link Credential}s.
 *
 * @author Bjoern Lohrmann, KNIME GmbH
 */
final class ScribeJavaHelper {

    private ScribeJavaHelper() {
    }

    static Credential fetchCredentialViaAuthCodeFlow(final CredentialsProvider credsProvider,
    ...
    static Credential fetchCredentialViaClientCredentialsFlow(final CredentialsProvider credsProvider,
            final BoxAuthenticatorSettings settings) throws Exception {

        try (var service = settings.createService(credsProvider)) {
            final var scribeJavaToken = new ClientCredentialsFlow(service).login(null);
            return CredentialFactory.fromScribeToken(scribeJavaToken, () -> settings.createService(credsProvider));
        }
    }
}
