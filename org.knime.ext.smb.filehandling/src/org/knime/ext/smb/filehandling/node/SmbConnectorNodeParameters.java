...
final class SmbConnectorNodeParameters implements NodeParameters {
    ...
    static final class AuthenticationParameters implements NodeParameters {
        ...
        static final String CFG_KEY_AUTH = "auth";
        private static final String TYPE_USER_PWD = "user_pwd";
        private static final String METHOD_KERBEROS = "kerberos";
        private static final String METHOD_GUEST = "guest";
        private static final String METHOD_ANONYMOUS = "anonymous";

        private static final String METHOD_USE_CREDENTIALS = "use_credentials";

        enum AuthenticationMethod {
            @Label(value = "Username & password", description = """
                    Authenticate with a username and password using NTLM. Either enter a username
                    and password, in which case the password will be persistently stored (in encrypted form)
                    with the workflow. The username field also accepts usernames of the form DOMAIN\\user
                    and user@DOMAIN.""")
            USERNAME_PASSWORD,

            @Label(value = "Use credentials", description = """
                    Authenticate with a username and password stored in a credentials flow
                    variable using NTLM.""")
            CREDENTIALS,
        ...
        static final class AuthenticationMethodPersistor implements NodeParametersPersistor<AuthenticationMethod> {
            ...
            public AuthenticationMethod load(final NodeSettingsRO settings) throws InvalidSettingsException {
                ...
                case TYPE_USER_PWD -> { // NOSONAR
                    final var useCredentials = settings//
                            .getNodeSettings(TYPE_USER_PWD)//
                            .getBoolean(METHOD_USE_CREDENTIALS, false);
                    yield useCredentials ? AuthenticationMethod.CREDENTIALS : AuthenticationMethod.USERNAME_PASSWORD;
                }
                case METHOD_KERBEROS -> AuthenticationMethod.KERBEROS;
                case METHOD_GUEST -> AuthenticationMethod.GUEST;
                case METHOD_ANONYMOUS -> AuthenticationMethod.ANONYMOUS;
            ...
            public void save(final AuthenticationMethod param, final NodeSettingsWO settings) {
                final var value = switch (param) {
                case USERNAME_PASSWORD, CREDENTIALS -> TYPE_USER_PWD;
                case KERBEROS -> METHOD_KERBEROS;
                case GUEST -> METHOD_GUEST;
                case ANONYMOUS -> METHOD_ANONYMOUS;
                };
            ...
            @Override
            public String[][] getConfigPaths() {
                return new String[][] { { CFG_KEY_TYPE }, { TYPE_USER_PWD, METHOD_USE_CREDENTIALS } };
            }
        }

        // ----- USER PASSWORD AUTHENTICATION SETTINGS -----

        @Persist(configKey = TYPE_USER_PWD)
        ...
        static final class UserPwdAuthParameters implements NodeParameters {
            ...
            @Widget(title = "Username & password", description = """
                    Authentication settings for username and password. Select "Use credentials"
                    as authentication \
                    method to provide the username and password via a credentials flow
                    variable.""")
            ...
            @Persist(configKey = METHOD_USE_CREDENTIALS)
            ...
            void validateOnConfigure(final CredentialsProvider cp) throws InvalidSettingsException {
                if (m_useCredentials) {
                    ...
                    } else if (StringUtils.isBlank(m_credentialsFlowVarName)) {
                        throw new InvalidSettingsException("No credentials flow variable selected.");
                    } else if (!cp.listNames().contains(m_credentialsFlowVarName)) {
                        throw new InvalidSettingsException(String.format(
                                "Required credentials flow variable '%s' is missing.", m_credentialsFlowVarName));
                    }
                } else {
                    if (StringUtils.isBlank(m_userPassword.getUsername())) {
                        throw new InvalidSettingsException("Username must be specified for authentication.");
                    }
                    // not validating password, as empty passwords might be valid in some setups
                }
            }
            ...
            static final class UserPasswordPersistor implements NodeParametersPersistor<Credentials> {
                ...
                private static final String CFG_KEY_USER = "user";
                private static final String CFG_KEY_PASSWORD = "password";

                @Override
                public Credentials load(final NodeSettingsRO settings) throws InvalidSettingsException {
                    final var user = settings.getString(CFG_KEY_USER, "");
                    final var password = settings.getPassword(CFG_KEY_PASSWORD, USER_PWD_ENCRYPTION_KEY, "");
                    return new Credentials(user, password);
                }

                @Override
                public void save(final Credentials param, final NodeSettingsWO settings) {
                    settings.addString(CFG_KEY_USER, param.getUsername());
                    settings.addPassword(CFG_KEY_PASSWORD, USER_PWD_ENCRYPTION_KEY, param.getPassword());
                }

                @Override
                public String[][] getConfigPaths() {
                    // See AP-14067: It is not possible to overwrite password fields
                    return new String[][] { { CFG_KEY_USER } };
                }
            }
        }

        void validate(final CredentialsProvider cp) throws InvalidSettingsException {
            if (m_type == AuthenticationMethod.CREDENTIALS || m_type == AuthenticationMethod.USERNAME_PASSWORD) {
                m_userPwdAuth.validateOnConfigure(cp);
            }
        }
    }
    ...
    void validateOnConfigure(final CredentialsProvider cp) throws InvalidSettingsException {
        ...
        m_authentication.validate(cp);

        if (StringUtils.isBlank(m_workingDirectory) || !m_workingDirectory.startsWith(SmbFileSystem.SEPARATOR)) {
    ...
    private void validateFileServerSettings() throws InvalidSettingsException {
        ...
        if (StringUtils.isBlank(m_fileserverShare)) {
    ...
    SmbFSConnectionConfig createFSConnectionConfig(final CredentialsProvider cp) {
        ...
        if (m_authentication.m_type == AuthenticationMethod.USERNAME_PASSWORD) {
            ...
            config.setUser(m_authentication.m_userPwdAuth.m_userPassword.getUsername());
            config.setPassword(
                    Optional.ofNullable(m_authentication.m_userPwdAuth.m_userPassword.getPassword()).orElse(""));
        } else if (m_authentication.m_type == AuthenticationMethod.CREDENTIALS) {
            config.setAuthType(SmbFSConnectionConfig.USER_PASSWORD_AUTH_TYPE);
            final var creds = cp.get(m_authentication.m_userPwdAuth.m_credentialsFlowVarName);
            config.setUser(creds.getLogin());
            config.setPassword(Optional.ofNullable(creds.getPassword()).orElse(""));
